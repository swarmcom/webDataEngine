...
'Paragraphs', 'DelimitedBlocks' and 'Table' content. filters.Filters are
primarily an extension mechanism for generating specialized outputs.
filters.Filters are implemented using external commands which are specified in
...
WARNING: filters.Filters sometimes included executable code. Before installing
...
~~~~~~~~~~~~~~~~~~~~~~~~~~
filters.Filters are normally accompanied by a configuration file containing a
